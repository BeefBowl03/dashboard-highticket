...
// Dealer Application API Endpoints
app.post('/api/dealer/generate', async (req, res) => {
  try {
...
app.post('/api/dealer/chat', async (req, res) => {
  try {
    ...
    const response = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        { role: 'system', content: 'You are a concise assistant for dealer applications. Help users with their dealer application process, provide suggestions, and answer questions about becoming a dealer.' },
        ...messages
      ],
      temperature: 0.2,
      max_tokens: 1000
